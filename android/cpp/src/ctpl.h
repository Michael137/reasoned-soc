	...
	void resize( int nThreads )
	{
		if( !this->isStop && !this->isDone )
		{
			...
			{ // the number of threads is decreased
				...
				this->threads.resize(
				    nThreads_ );                 // safe to delete because the threads are detached
				this->flags.resize( nThreads_ ); // safe to delete because the threads have copies
  ...
  private:
	...
	void set_thread( int i )
	{
		std::shared_ptr<std::atomic<bool>> flag(
		    this->flags[static_cast<size_t>( i )] ); // a copy of the shared ptr to the flag
		...
		this->threads[static_cast<size_t>( i )].reset(
