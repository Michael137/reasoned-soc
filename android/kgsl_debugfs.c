...
#define KGSL_LOGGING_DEFINE
#include "kgsl_device.h"
#undef KGSL_LOGGING_DEFINE
...
static ssize_t globals_write(struct file *file,
			    ...
			    size_t count, loff_t *position)
{
	...
	unsigned int len = 0;
	int logging_status;
	unsigned int buf_sz = 512;
	ssize_t ret = 0;

	status_string = kzalloc(buf_sz, GFP_KERNEL);
	if (!status_string)
		goto bail;
	...
	if (len > buf_sz)
		len = buf_sz;
	simple_read_from_buffer(buffer, count, position, status_string, len);
	kfree(status_string);
